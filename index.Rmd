...
* We then performed Propensity score subclassification on these imputed datasets, excluding individuals without similar counterparts (4.7%). That's what I'm showing in the figure on the right. These are the two propensity score distributions. the red is the aadolecents in Non-dis neighborhoods the blue is kids is dis neighborhoods. 
And the PS is the predicted probability of living in a disadvantaged neihgborhood given the observed covariates. 
...
Challenge: * Another challennge, as we talked about in the previous slide, is that Some kids with a history of severe or persistent adversity may experience blunting. This would have a washout effect on the association betweeh ND and cortisol 
...
* We then performed PS matching with replacement and Mahalanobis matching within calipers and exact matching as described on the previous slide. 
...
Top panel is for the pre-interview cortisol levels and Middle Panel is for the post-interview cortisol levels.
Bottom shows the conditional expected diffrences in cortisol slope during the late decline period comparing adolescents living in disadvantaged versus non-disadvantaged neighborhoods.
...
All adjusted models used adjusted for possible confounders listed in the previous figure in both propesntiy score matching and regression. BEcause of the definition of confounder, these were variables that we thought would be affected by the exposure. So for example, gender, age, race, season, region of the country, immigrant status, etc. 
...
-->
```{r, message=FALSE, echo=FALSE, comment="", echo=FALSE,  fig.width=13, fig.height=7}
...
scale_x_discrete(limits=c("trueTMLE", "trueDRWLS", "trueIPW", "trueIPTSvyW", "trueIPTW", "trueIPSW",  "Naive")) +
    ...
    coord_flip() + geom_hline(yintercept=ate, colour="red")  + ylab("PATE") +xlab("") + ggtitle("Scenario 1: linear effect heterogeneity")+ theme( title=element_text(size=9))
...
scale_x_discrete(limits=c("trueTMLE", "trueDRWLS", "trueIPW", "trueIPTSvyW", "trueIPTW", "trueIPSW",  "Naive")) +
    stat_summary(fun.y=mean, geom="point", shape=5, size=4) + 
    coord_flip() + geom_hline(yintercept=ate, colour="red")  + ylab("PATE") +xlab("") + ggtitle("Scenario 2: non-linear effect heterogeneity")+ theme( title=element_text(size=9))
...
grid.arrange(tmleS1.box.plot,tmleS2.box.plot,nrow=1)
```
